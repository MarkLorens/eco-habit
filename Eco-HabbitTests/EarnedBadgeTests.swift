...
/// The earned-badge card that pops wherever the user is standing.
///
/// Ported from Mark's `f573b04` on main, which fixed "badge icon not popping when
/// first unlocked". This branch already recorded awards but nothing announced
/// them, so the merge could have dropped the behaviour without a single conflict.
/// These tests are what makes that impossible to do quietly.
final class PendingBadgeTests: XCTestCase {

    @MainActor
    private func freshApp() async -> AppState {
        let app = AppState(userId: "pending-badge-user",
                           store: InMemoryKeyValueStore(),
                           seedDemoDataIfEmpty: false)
        await app.bootstrap()
        return app
    ...
    @MainActor
    func testNothingPendingOnAFreshAccount() async {
        let app = await freshApp()
        XCTAssertNil(app.pendingBadge)
    }

    @MainActor
    func testEarningABadgeMakesItPending() async {
        let app = await freshApp()
        await app.debugSetStreak(7)
        await app.logActivity(MockActivityData.all[0])

        XCTAssertEqual(app.pendingBadge?.id, "badge_streak_7")
    }

    /// The card shows once. Acknowledging is what retires it.
    @MainActor
    func testAcknowledgingClearsItAndItDoesNotComeBack() async {
        let app = await freshApp()
        await app.debugSetStreak(7)
        await app.logActivity(MockActivityData.all[0])

        guard let badge = app.pendingBadge else { return XCTFail("expected a pending badge") }
        await app.acknowledgeBadge(badge)

        XCTAssertNil(app.pendingBadge, "an acknowledged badge must not pop again")
        XCTAssertTrue(app.hasEarned(badge.id), "acknowledging must not un-earn it")
    }

    /// The reason this is persisted rather than held in memory: earn a badge,
    /// kill the app before looking at it, and it must still be waiting.
    @MainActor
    func testAPendingBadgeSurvivesARelaunch() async {
        let store = InMemoryKeyValueStore()

        let first = AppState(userId: "relaunch-user", store: store, seedDemoDataIfEmpty: false)
        await first.bootstrap()
        await first.debugSetStreak(7)
        await first.logActivity(MockActivityData.all[0])
        guard let badge = first.pendingBadge else { return XCTFail("expected a pending badge") }
        await first.acknowledgeBadge(badge)

        // Same store, new AppState — a relaunch.
        let second = AppState(userId: "relaunch-user", store: store, seedDemoDataIfEmpty: false)
        await second.bootstrap()
        XCTAssertNil(second.pendingBadge, "acknowledgement did not persist")
    }

    /// A `UserState` written before `announcedBadgeIds` existed must still decode.
    /// Synthesized `Decodable` ignores property defaults, so this field has to be
    /// Optional — get that wrong and every existing install loses its data.
    func testUserStateDecodesWithoutTheAnnouncedField() throws {
        // Encode a state that has the field, then strip exactly that one key —
        // which is what a file written by the previous build looks like.
        var current = UserState(userId: "legacy")
        current.announcedBadgeIds = ["badge_streak_7"]

        let encoded = try JSONEncoder().encode(current)
        guard var object = try JSONSerialization.jsonObject(with: encoded) as? [String: Any] else {
            return XCTFail("UserState did not encode to an object")
        }
        XCTAssertNotNil(object["announcedBadgeIds"], "precondition: the key is normally present")
        object.removeValue(forKey: "announcedBadgeIds")

        let legacy = try JSONSerialization.data(withJSONObject: object)
        let decoded = try JSONDecoder().decode(UserState.self, from: legacy)
        XCTAssertNil(decoded.announcedBadgeIds)
    }
}
