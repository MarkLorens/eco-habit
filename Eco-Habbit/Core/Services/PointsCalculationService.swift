...
//
//  Butuh   : Habit, PointsConfiguration
//  Dipakai : HabitRepository, unit test
//
//  Fungsi murni tanpa I/O: masukkan angka, dapat angka. Tidak menyentuh
//  repository maupun PersistedState, jadi bisa dites tanpa setup apa pun.
//

import Foundation

/// Rincian perhitungan satu pencatatan aksi.
/// Disimpan utuh ke `HabitLog` supaya poin di riwayat lama tetap bisa
...
nonisolated struct PointsCalculationService {
    ...
    func breakdown(
        ...
        let countedBase = min(habit.basePoints, remainingBaseToday)
    ...
    func basePointsUsed(in logs: [HabitLog]) -> Int {
