...
nonisolated struct Activity: Identifiable, Codable, Hashable {
    ...
    /// `Resources/activities.json` has no `hasEvidence` or `lastCompletedDate`
    /// — those are per-user progress, and a shared catalogue holds nobody's
    /// progress. Synthesised decoding would demand them and fail.
    ///
    /// `basePoints` is also optional in the file: omit it and the friction
    /// level supplies it, which is the rule the Swift table used.
