...
nonisolated struct UserState: Codable, Hashable {
    ...
    /// Badge ids whose "you earned this" card has already been shown, so it pops
    /// exactly once. Persisted rather than kept in memory: a badge earned and
    /// then force-quit before it was seen must still announce on the next open.
    ///
    /// **Optional on purpose.** Synthesized `Decodable` ignores property default
    /// values — a missing key throws `keyNotFound`, not a fallback to `[]` — so a
    /// non-optional field here would fail to decode every `UserState` already
    /// saved on a device. Optional gets `decodeIfPresent` and reads as `nil`.
    var announcedBadgeIds: Set<String>? = nil
