...
struct Fight: Identifiable, Codable, Hashable {
    ...
    /// reads this is also the habit scanner, so it is pointed at arbitrary
    /// scenes all day — posters, menus, wifi codes. Encoding the bare code would
    /// force it to try every QR in the world against the fight list, and a
    /// six-character collision would silently check somebody into an event they
    /// are nowhere near. Anything not starting with `ecohabit://fight/` is
    /// discarded before it reaches any logic.
    ...
    ///
    /// Deliberately permissive about *case* and nothing else: a QR generated by
    /// this app is exact, but a scanner or a copy-paste can change case, and the
    /// code itself is normalised again by `matchesCheckInCode`.
